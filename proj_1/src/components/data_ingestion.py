...
class DataIngestion:
    def __init__(self):
        # 1.Load Config and define all paths dynamically
        self.config = load_config(get_config_path('paths_config.yaml'))

        # Construct specific file paths
        c = self.config  # Shorthand for config

        self.raw_path = get_data_path(c['RAW_DIR']) / c['RAW_DATA_FILENAME']
        self.train_path = get_data_path(c['PROCESSED_DIR']) / c['TRAIN_DATA_FILENAME']
        self.test_path = get_data_path(c['PROCESSED_DIR']) / c['TEST_DATA_FILENAME']
    ...
    def initiate_data_ingestion(self):
        ...
        try: 
            ...
            # Split data using parameters from the config
            ...
            # Return string versions of paths for downstream modules
